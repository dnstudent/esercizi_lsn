...
/**
 * The exercise integrand sampling with a uniform distribution.
 * @param x x
 */
value f1(const value x) { return M_PI_2 * cos(x * M_PI_2); }

/**
 * The exercise integrand sampling with a normal distribution.
 * @param x x
...
int main(int argc, char const *argv[]) {
    ...
    if (N_THROWS % N_BLOCKS != 0) {
        std::cout << "Must choose a number of blocks which divides the number of throws"
                  << std::endl;
        return 1;
    }
    ...
    // The block estimator
    estimators::StatefulMean<value> integral_estimator;

    // .1 Sample using the uniform distribution. Range (-1, 1) will be used instead of (0,1)
